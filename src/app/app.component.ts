...
export class AppComponent {
  ...
  presentItems:Array<number> = []
  total:number = 0
  
  insertProduct(e:Product){
    if(this.presentItems.indexOf(e.id) == -1)
    {
      this.presentItems.push(e.id)
      console.log(e.id)
  ...
  set searchvalue(value : string){
    ...
    this.products.forEach((obj)=>{
      if(obj.title.indexOf(this.searchvalue)!=-1)
      {
        this.searchedproducts.push(obj)
      }
    })
  }
  removeItem(id:Number){
    ...
    this.total = Math.floor(this.total - this.cartItems[itemIndex].price.value)
  ...
  decrease(id:Number){
    ...
    let itemIndex = this.cartItems.findIndex(obj => obj.id === id)
    this.total = Math.floor(Math.floor(this.total) - Math.floor(this.cartItems[itemIndex].price.value))
  }
  increase(id:Number){
    let itemIndex = this.cartItems.findIndex(obj => obj.id === id)
    this.total = Math.floor(Math.floor(this.total) + Math.floor(this.cartItems[itemIndex].price.value))
